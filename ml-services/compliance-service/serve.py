...
Using trained BERT + Random Forest clause detector (100% accuracy)
"""
...
app = FastAPI(
    ...
    description="AI-powered contract and document compliance checking with BERT",
    version="2.0.0"
)
...
def load_models():
    """Load trained BERT model and classifier"""
    try:
        ...
        # Load classifier
        MODEL_ARTIFACTS['classifier'] = joblib.load('compliance_classifier.pkl')
        ...
        logger.info(f"   BERT Model: {MODEL_ARTIFACTS['config']['bert_model_name']}")
        logger.info(f"   Accuracy: {MODEL_ARTIFACTS['config']['accuracy']*100:.2f}%")
        logger.info(f"   Clause Types: {len(MODEL_ARTIFACTS['clause_types'])}")
        
    except Exception as e:
        logger.error(f"❌ Error loading models: {e}")
        logger.error("Please run train_compliance_model.py first!")
        raise
...
def detect_clauses_ml(document_text: str) -> Dict[str, Any]:
    """Detect clauses using trained BERT model"""
    ...
    probabilities = classifier.predict_proba(sentence_embeddings)
...
def check_document_compliance(document_text: str, document_id: str = None) -> Dict[str, Any]:
    """Comprehensive compliance check using ML model"""
    
    logger.info(f"🔍 Analyzing document: {document_id or 'N/A'}")
    
    # Detect clauses using trained model
    ...
    return {
        ...
        'model_info': {
            ...
            'version': MODEL_ARTIFACTS['config']['model_version'],
            'accuracy': f"{MODEL_ARTIFACTS['config']['accuracy']*100:.2f}%",
            'trained_on': MODEL_ARTIFACTS['config']['training_date'][:10]
        }
    }
...
async def health():
    ...
    return {
        ...
        "model_accuracy": MODEL_ARTIFACTS.get('config', {}).get('accuracy', 0) * 100 if model_loaded else 0
    }

@app.post("/check")
async def check_compliance(request: DocumentRequest):
    """Check a single document for compliance using ML model"""
    try:
...
async def check_batch_compliance(request: BatchDocumentRequest):
    ...
    try:
        ...
        batch_summary = {
            ...
            'timestamp': datetime.now().isoformat(),
            'model_used': 'BERT + Random Forest Ensemble'
        }
...
if __name__ == "__main__":
    ...
    print("📊 Using trained BERT + Random Forest model")
